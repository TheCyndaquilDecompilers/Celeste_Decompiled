...
namespace Celeste
{
	...
	{
		// Token: 0x060018F9 RID: 6393 RVA: 0x0009CB6C File Offset: 0x0009AD6C
		public static void Load()
		{
			Chooser<MTexture> sourceChooser = new Chooser<MTexture>(new MTexture[]
			{
			...
			Chooser<MTexture> sourceChooser2 = new Chooser<MTexture>(new MTexture[]
			{
			...
			ParticleTypes.SparkyDust = new ParticleType(ParticleTypes.Dust)
			{
				SourceChooser = sourceChooser2,
			...
			StarTrackSpinner.P_Trail = new ParticleType[]
			{
				particleType,
			...
			foreach (KeyValuePair<string, Color> keyValuePair in ForsakenCitySatellite.Colors)
			{
				ForsakenCitySatellite.Particles.Add(keyValuePair.Key, new ParticleType(Player.P_DashA)
				{
					Color = keyValuePair.Value,
					Color2 = Color.Lerp(keyValuePair.Value, Color.White, 0.5f)
				});
			}
